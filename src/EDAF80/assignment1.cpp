...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
        ...
        CelestialBodyRef start;
        start.body = &sun;
        start.parent_transform = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f, 0.0f));
        stack.push(start);
        while(!stack.empty())
        {
            ...
            for (int i = 0; i < children.size(); i++)
            {
                ...
                CelestialBodyRef child;
                child.body = children[i];
                child.parent_transform = transformation;
                stack.push(child);
            }
        }
		//
